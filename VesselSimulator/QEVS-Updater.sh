...
wget $(curl -s https://api.github.com/repos/CYBUTEK/KerbalEngineer/releases/latest | jq -r ".zipball_url") -O latest.zip -q
...
BuildAdvanced=$(echo "$BuildAdvanced" | sed -r 's:using (Extensions|Flight|Helpers|KeyBinding|Settings|UIControls|VesselSimulator)://using \1:g') && echo "BuildAdvanced.cs néttoyage des imports réussi"
